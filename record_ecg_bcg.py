"""
ECG + BCG live recorder (Windows + Arduino Uno R3/R4)
-----------------------------------------------------
Reads CSV lines from an Arduino running `ecg_bcg_sampler.ino`, shows a
scrolling live plot of both channels, and saves all samples to a
timestamped CSV file. Stop recording by closing the plot window or
...
Find your Arduino's COM port:
    py record_ecg_bcg.py --list

Record a session (example, replace COM5 with your actual port):
    py record_ecg_bcg.py --port COM5 --label standing
    py record_ecg_bcg.py --port COM5 --label sitting
...
SAMPLE_RATE_HZ   = 200
PLOT_WINDOW_S    = 10                                  # seconds of history on screen
PLOT_BUFFER_N    = SAMPLE_RATE_HZ * PLOT_WINDOW_S
VREF             = 5.0                                 # Arduino analog reference (volts)
PLOT_REFRESH_HZ  = 30                                  # screen update rate
# -----------------------------------------------------------
...
def main():
    parser = argparse.ArgumentParser(description="Record ECG + BCG from Arduino")
    ...
    time.sleep(2)              # Arduino resets when the port opens; wait for boot
    ...
    writer.writerow(["t_seconds", "bcg_volts", "ecg_volts", "bcg_raw", "ecg_raw"])
    print(f"Recording to {csv_path}")
    print("Close the plot window (or press Ctrl-C) to stop.\n")

    # --- Ring buffers for the live plot ---
    t_buf   = deque(maxlen=PLOT_BUFFER_N)
    bcg_buf = deque(maxlen=PLOT_BUFFER_N)
    ecg_buf = deque(maxlen=PLOT_BUFFER_N)

    # --- Set up the live plot ---
    plt.ion()
    fig, (ax_bcg, ax_ecg) = plt.subplots(2, 1, sharex=True, figsize=(11, 6))
    fig.suptitle("Live ECG + BCG   (close window to stop)")

    (line_bcg,) = ax_bcg.plot([], [], lw=0.9)
    (line_ecg,) = ax_ecg.plot([], [], lw=0.9, color="tab:orange")

    ax_bcg.set_ylabel("BCG (V)")
    ax_ecg.set_ylabel("ECG (V)")
    ax_ecg.set_xlabel("Time (s)")
    for ax in (ax_bcg, ax_ecg):
        ax.grid(True, alpha=0.3)

    # Detect when the user closes the window
    running = {"go": True}
    ...
    try:
        while running["go"]:
            # --- Drain all waiting serial lines so we don't fall behind ---
            while ser.in_waiting and running["go"]:
                ...
                ecg_v = (ecg_raw / adc_max) * VREF

                writer.writerow([f"{t_s:.6f}", f"{bcg_v:.5f}", f"{ecg_v:.5f}",
                                 bcg_raw, ecg_raw])
            ...
            # --- Redraw at a capped rate (plotting is the slow part) ---
            now = time.monotonic()
            if now - last_plot_time >= plot_period and len(t_buf) > 2:
                line_bcg.set_data(t_buf, bcg_buf)
                line_ecg.set_data(t_buf, ecg_buf)

                t_right = t_buf[-1]
                t_left  = max(0.0, t_right - PLOT_WINDOW_S)
                ax_bcg.set_xlim(t_left, t_right)

                # Autoscale y with a small margin
                for ax, buf in ((ax_bcg, bcg_buf), (ax_ecg, ecg_buf)):
            ...
            # --- Periodic status in the terminal ---
            if now - last_status_time >= 2.0:
                elapsed = (t_buf[-1] if t_buf else 0.0)
                print(f"  t = {elapsed:6.1f} s   samples = {sample_count}", end="\r")
                last_status_time = now

            # Small yield so matplotlib stays responsive without burning CPU
            plt.pause(0.005)
